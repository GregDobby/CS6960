...
def main():
    ...
    ## encoder
    # layer 1
    ew1 = tf.Variable(tf.random_normal([enc_layer_size[0], enc_layer_size[1]], stddev=math.sqrt(1 / (enc_layer_size[0]))))
    eb1 = tf.Variable(tf.zeros(enc_layer_size[1]))
    ey1 = tf.nn.relu(tf.matmul(x_enc, ew1) + eb1)

    # layer 2
    ew2 = tf.Variable(tf.random_normal([enc_layer_size[1], enc_layer_size[2]], stddev=math.sqrt(1 / (enc_layer_size[1]))))
    eb2 = tf.Variable(tf.zeros(enc_layer_size[2]))
    ey2 = tf.nn.relu(tf.matmul(ey1, ew2) + eb2)

    # layer 3
    ew3 = tf.Variable(tf.random_normal([enc_layer_size[2], enc_layer_size[3]], stddev=math.sqrt(1 / (enc_layer_size[2]))))
    eb3 = tf.Variable(tf.zeros(enc_layer_size[3]))
    ey3 = tf.nn.relu(tf.matmul(ey2, ew3) + eb3)

    # layer 4
    ew4 = tf.Variable(tf.random_normal([enc_layer_size[3], enc_layer_size[4]], stddev=math.sqrt(1 / (enc_layer_size[3]))))
    ...
    ## decoder
    # layer 1
    dw1 = tf.Variable(tf.random_normal([dec_layer_size[0], dec_layer_size[1]], stddev=math.sqrt(1 / (dec_layer_size[0]))))
    db1 = tf.Variable(tf.zeros(dec_layer_size[1]))
    dy1 = tf.nn.relu(tf.matmul(g_dec, dw1) + db1)

    
    # layer 2
    dw2 = tf.Variable(tf.random_normal([dec_layer_size[1], dec_layer_size[2]], stddev=math.sqrt(1 / (dec_layer_size[1]))))
    db2 = tf.Variable(tf.zeros(dec_layer_size[2]))
    dy2 = tf.nn.relu(tf.matmul(dy1, dw2) + db2)

    # layer 3 
    dw3 = tf.Variable(tf.random_normal([dec_layer_size[2], dec_layer_size[3]], stddev=math.sqrt(1 / (dec_layer_size[2]))))
    db3 = tf.Variable(tf.zeros(dec_layer_size[3]))
    dy3 = tf.nn.relu(tf.matmul(dy2, dw3) + db3)

    # layer 4
    dw4 = tf.Variable(tf.random_normal([dec_layer_size[3], dec_layer_size[4]], stddev=math.sqrt(1 / (dec_layer_size[3]))))
    db4 = tf.Variable(tf.zeros(dec_layer_size[4]))
    g_decoded = tf.matmul(dy3, dw4) + db4

    # loss
    loss = tf.reduce_mean(((g - g_decoded)/g)**2)
