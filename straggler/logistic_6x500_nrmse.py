...
def main():
   ...
   # enc_layer_size = [dim * nw * npt, hn, hn, hn, dim * npt] # encoder layer sizes
    # 4 hidden layer
    enc_layer_size = [dim * nw * npt,hn,hn, hn, hn, hn, dim * npt] # encoder layer sizes
#    enc_layer_act = [tf.nn.tanh, tf.nn.tanh, tf.nn.tanh, None]
    enc_layer_act = [tf.nn.relu, tf.nn.relu,tf.nn.relu, tf.nn.relu,tf.nn.relu, None]
   # dec_layer_size = [dim * (nw + nr), hn, hn, hn, nu * dim] # decoder layer sizes
    dec_layer_size = [dim * (nw + nr), hn, hn, hn,hn, hn, nu * dim]
 #   dec_layer_act = [tf.nn.tanh, tf.nn.tanh, tf.nn.tanh, None]
    dec_layer_act = [tf.nn.relu, tf.nn.relu, tf.nn.relu,tf.nn.relu, tf.nn.relu, None]
    ## encoder
    # encoder input
    ...
    # loss
    t1 = tf.reduce_sum((m - dec_param[n_hidden_layer - 1]['y'])**2, axis = 1)
    t2 = tf.reduce_sum(m**2, axis =1)
    loss = tf.sqrt(tf.reduce_mean(t1 / t2))
    # optimizer
    ...
    with tf.Session() as sess:
        ...
        for i in range(n_hidden_layer):
            # weight
       	    w = sess.run(enc_param[i]['w']) 
            # bias
            b = sess.run(enc_param[i]['b'])
            t= np.concatenate((w,np.reshape(b,(1,-1))), axis=0)
            np.savetxt('layer-'+str(i)+'.csv', t.T)
