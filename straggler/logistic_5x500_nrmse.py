...
def main():
   ...
   # enc_layer_size = [dim * nw * npt, hn, hn, hn, dim * npt] # encoder layer sizes
    # 4 hidden layer
    enc_layer_size = [dim * nw * npt,hn, hn, hn, hn, dim * npt] # encoder layer sizes
#    enc_layer_act = [tf.nn.tanh, tf.nn.tanh, tf.nn.tanh, None]
    enc_layer_act = [tf.nn.relu, tf.nn.relu, tf.nn.relu,tf.nn.relu, None]
   # dec_layer_size = [dim * (nw + nr), hn, hn, hn, nu * dim] # decoder layer sizes
    dec_layer_size = [dim * (nw + nr), hn, hn, hn, hn, nu * dim]
 #   dec_layer_act = [tf.nn.tanh, tf.nn.tanh, tf.nn.tanh, None]
    dec_layer_act = [tf.nn.relu, tf.nn.relu, tf.nn.relu, tf.nn.relu, None]
    ## encoder
    # encoder input
